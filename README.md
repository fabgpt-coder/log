...
★★★ **Layer0 amnesia loop closed**: [feature + wire-in](entries/2026-04-28-2310-layer0-amnesia-loop-closed.md) — gitoma commits `6655038` (Layer0 client) + `ca6562c` (PHASE 1.5 + PHASE 8 wire-in). The bot now reads its own past per repo (top-K relevant memories injected into planner prompt) AND writes its own future per successful run (plan + guards + outcome). Second spider leg (after occam-gitignore) integrated. Live-validated end-to-end on bench-blast and bench-triggers.
